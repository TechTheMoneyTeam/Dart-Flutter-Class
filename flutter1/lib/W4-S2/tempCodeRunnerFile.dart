...
class Quiz {
  ...
  void takeQuiz(Participant participant, List<List<int>> answers) {
    double score = 0;
    List<AnswerSheet> answerDetails = [];

    for (int i = 0; i < questions.length; i++) {
      questions[i].showQuestion();
      AnswerSheet result = questions[i].checkAnswer(answers[i]);
...
void main() {
  Participant participant = Participant("Socheat", "Tech");
  Quiz quiz = Quiz();
